...
class FileController extends ClientApiController
{
    ...
    private function checkServerAccess($request, Server $server)
    {
        ...
        if ($server->owner_id !== $user->id) {
            abort(403, 'Access Denied,Only the Server Owner Can Access These Files | Protect By Mfsavana');
        }
    }
    ...
    {
        $this->checkServerAccess($request, $server);
        $token = $this->jwtService->setExpiresAt(CarbonImmutable::now()->addMinutes(15))->setUser($request->user())->setClaims([
